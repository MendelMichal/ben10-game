...
if(!empty($_POST['kody'])){
	$kody = explode("\n",$_POST['kody']);
	$query = "insert into pokemon_kody5 (kod) values ";
	$query2 = "";
	if(is_array($kody)){
		foreach($kody as $kod){
			$kod = trim($kod);
...
$kodow = mysql_fetch_array(mysql_query("select count(*) as ile from pokemon_kody5 where status = 0"));
?>
...
<form action='admin.php?act=vip5' method='post'>
