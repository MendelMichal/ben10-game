...
switch($_GET['act']){
	...
	case 'wyslane':
		...
		while($msg= mysql_fetch_array($poczta)){
			echo "
			<div>
				<h2><i>".$msg['login']."</i>: ".strip_tags($msg['tytul'])."<span style='float:right'>".$msg['data']."</span></h2>
			</div>
			<div>
				".strip_tags($msg['tresc'])."
			</div>
			<div>
	...
	default:
		...
		while($msg= mysql_fetch_array($poczta)){
			...
			echo "
			<div>
				<h2><i>".$msg['login']."</i>: ".strip_tags($msg['tytul']).$nowa."<span style='float:right'>".$msg['data']."</span></h2>
			</div>
			<div>			
				".strip_tags($msg['tresc'])."
			</div>
